...
UserLicenseAgreement() {
	...
	if [ "$user_input" = "y" ] || [ "$user_input" = "Y" ]; then
		[ ! -L "$PROJECT_STATE_DIR" ] || { echo "拒绝使用符号链接状态目录。"; exit 1; }
		mkdir -p -- "$PROJECT_STATE_DIR" || exit 1
		[ -O "$PROJECT_STATE_DIR" ] || { echo "状态目录不属于当前用户。"; exit 1; }
  ...
  while true; do
		  ...
		  case $sub_choice in
			  ...
			  3)
				  ...
				  ;;
			  4)
				  echo -e "${gl_hong}安全策略已禁用清空规则后关闭全部端口的操作。${gl_bai}"
				  echo "请使用 close_port 逐个关闭端口。"
				  ;;
...
format_partition() {
	...
	read -e -r -p "此操作不可恢复。请输入 $confirmation_token 以确认: " CONFIRM
	  ...
	  case $sub_choice in
		  1)
			  while true; do
				  ...
				  install_project_entrypoint || {
					  echo -e "${gl_hong}安装主命令失败，未创建快捷键。${gl_bai}"
					  break_end
					  continue
				  }
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path_interactive "$dirname" && echo "目录已删除" || echo "删除失败或已取消"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
...
run_commands_on_servers() {
	...
	for ((i=0; i<${#SERVER_ARRAY[@]}; i+=5)); do
		...
		if [ -n "$stored_password" ]; then
			echo -e "${gl_hong}检测到旧版明文密码字段；该值已忽略，请编辑 servers.py 将其清空。${gl_bai}"
		fi
		kj_ssh_read_password "请输入 $username@$hostname 的密码: "
