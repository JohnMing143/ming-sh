...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# モード:" "$ACTIVE_OPTIMIZE_CONFIG_PATH" 2>/dev/null | sed 's/# モード: //' | awk -F'|' '{print $1}' | xargs)
...
EOF
		  ...
		  102)
			  clear
			  send_stats "${PROJECT_NAME} をアンインストールする"
...
command_help() {
send_stats "${PROJECT_COMMAND} コマンドリファレンスの使用例"
echo "-------------------"
