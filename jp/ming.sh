...
format_partition() {
	...
	# フォーマットの確認
	read -e -r -p "パーティション $DEVICE を $FS_TYPE でフォーマットしますか？(y/n): " CONFIRM
	if [[ ! "$CONFIRM" =~ ^[Yy]$ ]]; then
		echo "操作はキャンセルされました。"
		return
	fi
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path "$dirname" && echo "ディレクトリが削除されました" || echo "削除に失敗しました"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
