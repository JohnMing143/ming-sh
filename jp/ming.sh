		...
		1)
			...
			if [ "${#docker_command[@]}" -lt 2 ] ||
			   ...
			   { [ "${docker_command[1]}" != "run" ] && [ "${docker_command[1]}" != "create" ]; }; then
				echo -e "${gl_hong}docker run または docker create コマンドのみ実行できます。${gl_bai}"
			else
				"${docker_command[@]}"
			fi
			;;
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}ヒント：${gl_bai}この機能を実行するには root ユーザーが必要です。" && break_end && return_to_main_menu
}
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 ${PROJECT_NAME} による OPENCLAW 管理ツール 🦞"
...
EOF
	...
	openclaw_memory_view_file() {
		...
		read -e -p "開始行を入力してください (Enter キーを押すと末尾 ${default_lines} 行がデフォルトになります): " start_line
		read -e -p "表示する行数を入力してください (Enter でデフォルト ${default_lines}): " count
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "設定${docker_name}ログイン キー (sk- で始まる文字と数字の組み合わせ) 例: sk-example159163:" app_passwd
...
linux_work() {
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  23)
			  read -e -p "バックグラウンドで実行するコマンドを入力してください（例: uptime）:" tmuxd
			  tmux_run_d
			  ;;
		  ...
		  1)
			  while true; do
				  ...
				  if [[ ! "$kuaijiejian" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || [[ "$kuaijiejian" == *..* ]]; then
					  echo -e "${gl_hong}ショートカット名には英数字、ドット、アンダースコア、ハイフンのみ使用でき、連続したドットは使用できません。${gl_bai}"
					  break_end
					  continue
				  fi
				  ...
				  for shortcut_path in "/usr/local/bin/$kuaijiejian" "/usr/bin/$kuaijiejian"; do
					  if [ -e "$shortcut_path" ] || [ -L "$shortcut_path" ]; then
						  ...
						  if [ "$shortcut_target" != "$project_install_target" ]; then
							  echo -e "${gl_hong}既存のコマンドは上書きしません: $shortcut_path${gl_bai}"
				  ...
				  install_project_entrypoint || {
					  echo -e "${gl_hong}コマンドのインストールに失敗しました。${gl_bai}"
					  break_end
					  continue
				  }
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "新しい解析レコード形式を入力してください: 110.25.5.33 example.com:" addhost
		  ...
		  102)
			  clear
			  echo "${PROJECT_NAME} をアンインストールする"
			  echo "------------------------------------------------"
			  echo "${PROJECT_NAME} は、他の機能に影響を与えることなく完全にアンインストールされます。"
			  read -e -p "続行してもよろしいですか? (はい/いいえ):" choice
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path_interactive "$dirname" && echo "ディレクトリが削除されました" || echo "削除に失敗しました"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
...
run_commands_on_servers() {
	...
	for ((i=0; i<${#SERVER_ARRAY[@]}; i+=5)); do
		...
		if [ -n "$stored_password" ]; then echo -e "${gl_hong}旧形式の平文パスワードは無視されました。servers.py から削除してください。${gl_bai}"; fi
		kj_ssh_read_password "$username@$hostname のパスワード: "
...
while true; do
	  ...
	  case $sub_choice in
		  1)
			  ...
			  if [[ ! "$server_name" =~ ^[A-Za-z0-9._-]+$ ]] || [[ ! "$server_ip" =~ ^[A-Za-z0-9._:-]+$ ]] || [[ ! "$server_port" =~ ^[0-9]+$ ]] || [ "$server_port" -lt 1 ] || [ "$server_port" -gt 65535 ] || [[ ! "$server_username" =~ ^[A-Za-z0-9._-]+$ ]]; then echo "サーバー情報が無効です。"; break_end; continue; fi
		  ...
		  2)
			  ...
			  if [[ "$rmserver" =~ ^[A-Za-z0-9._-]+$ ]]; then sed -i "/\"name\": \"$rmserver\"/d" "$HOME/cluster/servers.py"; else echo "サーバー名が無効です。"; fi
			  ;;
		  3)
			  install nano
...
echo "以下は、${PROJECT_COMMAND} コマンドの参考使用例です。"
echo "スクリプト${PROJECT_COMMAND}を開始します"
echo "パッケージをインストールします ${PROJECT_COMMAND} install nano wget | ${PROJECT_COMMAND} ナノ wget を追加 | nano wgetをインストールします"
echo "パッケージをアンインストールします。 ${PROJECT_COMMAND} 削除 nano wget | ${PROJECT_COMMAND}デルナノwget | nano wget をアンインストールする | nano wgetをアンインストールします"
echo "システム ${PROJECT_COMMAND} アップデートを更新 | ${PROJECT_COMMAND}アップデート"
echo "クリーン系ジャンククリーン |きれいだ"
echo "システムパネルを再度取り付けます。 ${PROJECT_COMMAND}再インストール"
echo "BBR3 コントロール パネル K BBR3 | ${PROJECT_COMMAND} bbrv3"
echo "カーネル チューニング パネル${PROJECT_COMMAND} カーネルの最適化"
echo "仮想メモリ ${PROJECT_COMMAND} スワップを設定 2048"
echo "仮想タイムゾーンを設定します ${PROJECT_COMMAND} 時間 アジア/上海 | ${PROJECT_COMMAND} タイムゾーン アジア/上海"
echo "システムごみ箱のゴミ箱 | ${PROJECT_COMMAND} hz | ${PROJECT_COMMAND} ごみ箱"
echo "システムバックアップ機能 ${PROJECT_COMMAND}バックアップ | ${PROJECT_COMMAND} bf | ${PROJECT_COMMAND} バックアップ"
echo "ssh リモート接続ツール ${PROJECT_COMMAND} ssh | ${PROJECT_COMMAND} リモート接続"
echo "rsync リモート同期ツール ${PROJECT_COMMAND} rsync | ${PROJECT_COMMAND} リモート同期"
echo "ハードディスク管理ツール ${PROJECT_COMMAND} ディスク | ${PROJECT_COMMAND} ハードディスクの管理"
echo "イントラネット普及率 (サーバー) ${PROJECT_COMMAND} frps"
echo "イントラネット浸透率 (クライアント) ${PROJECT_COMMAND} frpc"
echo "ソフトウェア起動 ${PROJECT_COMMAND} start sshd | sshdを起動します"
echo "ソフトウェア停止 ${PROJECT_COMMAND} 停止 sshd | ${PROJECT_COMMAND} ストップ sshd"
echo "ソフトウェア再起動 ${PROJECT_COMMAND} 再起動 sshd | ${PROJECT_COMMAND} sshdを再起動します"
echo "ソフトウェアのステータスを確認します。 ${PROJECT_COMMAND} ステータス sshd | ${PROJECT_COMMAND}ステータスsshd"
echo "${PROJECT_COMMAND} ドッカーを有効にする | ${PROJECT_COMMAND} 自動開始ドッカー | ${PROJECT_COMMAND} ソフトウェアの起動時に Docker を有効にする"
echo "ドメイン名証明書アプリケーション ${PROJECT_COMMAND} ssl"
echo "ドメイン名証明書の有効期限のクエリ ${PROJECT_COMMAND} ssl ps"
echo "docker 管理プレーン ${PROJECT_COMMAND} docker"
echo "docker 環境のインストール ${PROJECT_COMMAND} docker install |${PROJECT_COMMAND} docker インストール"
echo "docker コンテナ管理 ${PROJECT_COMMAND} docker ps |${PROJECT_COMMAND} docker コンテナ"
echo "docker イメージ管理 ${PROJECT_COMMAND} docker img |${PROJECT_COMMAND} docker image"
echo "LDNMP サイト管理 ${PROJECT_COMMAND} Web"
echo "LDNMP キャッシュのクリーニング ${PROJECT_COMMAND} Web キャッシュ"
echo "WordPress をインストールします。 ${PROJECT_COMMAND}ワードプレス | ${PROJECT_COMMAND} wp xxx.com"
echo "リバース プロキシをインストールします ${PROJECT_COMMAND} fd |${PROJECT_COMMAND} rp |${PROJECT_COMMAND} リバース プロキシ |${PROJECT_COMMAND} fd xxx.com"
echo "ロード バランシングのインストール ${PROJECT_COMMAND} ロード バランシング |${PROJECT_COMMAND} ロード バランシング"
echo "L4 ロード バランシング ${PROJECT_COMMAND} ストリーム |${PROJECT_COMMAND} L4 ロード バランシングをインストールする"
echo "ファイアウォール パネル ${PROJECT_COMMAND} fhq |${PROJECT_COMMAND} ファイアウォール"
echo "ポートを開きます ${PROJECT_COMMAND} dkdk 8080 |${PROJECT_COMMAND} ポートを開きます 8080"
echo "ポート ${PROJECT_COMMAND} gbdk 7800 を閉じる |${PROJECT_COMMAND} ポート 7800 を閉じる"
echo "リリース IP ${PROJECT_COMMAND} fxip 127.0.0.0/8 |${PROJECT_COMMAND} リリース IP 127.0.0.0/8"
echo "ブロック IP ${PROJECT_COMMAND} zzip 177.5.25.36 |${PROJECT_COMMAND} ブロック IP 177.5.25.36"
echo "コマンド お気に入り ${PROJECT_COMMAND} お気に入り | ${PROJECT_COMMAND} コマンドのお気に入り"
echo "アプリケーションマーケット管理${PROJECT_COMMAND}アプリ"
echo "申請番号の迅速な管理 ${PROJECT_COMMAND} app 26 | ${PROJECT_COMMAND}アプリ1パネル | ${PROJECT_COMMAND} アプリ npm"
echo "フェイル 2 バン管理 ${PROJECT_COMMAND} フェイル 2 バン | ${PROJECT_COMMAND} f2b"
echo "システム情報を表示 ${PROJECT_COMMAND} info"
echo "ROOT キー管理 ${PROJECT_COMMAND} sshkey"
echo "SSH 公開キーのインポート (URL) ${PROJECT_COMMAND} sshkey <url>"
echo "SSH 公開キーのインポート (GitHub) ${PROJECT_COMMAND} sshkey github <user>"
		...
		sshkey)
			...
			case "$1" in
				...
				* )
					echo "エラー: 不明なパラメータ '$1'"
					echo "使用法："
					echo "${PROJECT_COMMAND} sshkey は対話型メニューに入ります"
					echo "${PROJECT_COMMAND} sshkey \"<pubkey>\" SSH 公開キーを直接インポートします"
					echo "${PROJECT_COMMAND} sshkey <url> URL から SSH 公開キーをインポートします"
					echo "${PROJECT_COMMAND} sshkey github <user> GitHub から SSH 公開キーをインポートします"
					;;
			esac
