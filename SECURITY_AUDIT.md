...
| CMD-013 | High: dependencies are mutable and usually unpinned | Installers use `latest`, branch heads, short domains, mutable images, and remote application definitions. The remote-script validator displays the fetched SHA-256 but does not compare it with a repository-pinned value. A 2026-07-19 review of the whole `run_reviewed_remote_script` surface (19 URLs) found static script pinning is not applicable: 14 target third-party moving refs (`latest` release assets or `main`/`master` branch heads) that change by design, and the 5 project-owned URLs interpolate the proxy-dependent `PROJECT_DOWNLOAD_BASE`. The OpenSSH helper uses signed distribution packages instead of source tarballs. | A future upstream change can alter behavior without a repository change and can execute as soon as its feature is selected. | Open, and reclassified 2026-07-19: script-level pinning does not fit a deliberately mutable surface. The remaining, applicable integrity work is pinning container **images** and application **releases** to digests (a separate mechanism from the script validator), and is the actual CMD-013 remediation going forward. |
...
1. Pin mutable container **images**, archives, and application **releases** to
   reviewed digests. (Script-level pinning was assessed on 2026-07-19 and found
   inapplicable: every `run_reviewed_remote_script` target is a deliberately
   mutable ref — third-party `latest`/branch heads or a proxy-dependent
   project base — so there is no stable digest to pin. HTTPS, syntax
   validation, and digest display remain the transport/transparency controls;
   images and releases are where a repository-known integrity value applies.)
