...
Audit date: 2026-07-14 (updated 2026-08-18: CMD-019/CMD-020 added and resolved,
CMD-013 URL count corrected, self-update mechanism clarified)
...
- Project self-update is disabled by the `project_update` stub (no download,
  cron, Git, or replacement operation) plus its static test guard; the
  `ENABLE_SELF_UPDATE=false` default documents intent but has no consumer.
...
- Remote shell installers are downloaded over verified HTTPS into a private
  cache, Bash syntax-checked, and logged with their source and SHA-256 digest
  where available. Project-owned scripts that must persist (cron/tmux/profile
  jobs: certificate renewal, CF-Under-Attack, traffic-limit shutdown, the TG
  monitor pair) are installed to their target path by
  `download_reviewed_remote_script --install` after the same checks instead
  of a raw `curl`; one-shot installers are executed automatically by
  `run_reviewed_remote_script` and then removed. No additional approval
...
| CMD-010 | High: credentials are stored or displayed in plaintext | The cluster feature now offers SSH-key authentication: leaving the add-server password blank stores the credential `key` (or a hand-set `key:/path`), and `run_commands_on_servers` then connects with `ssh` using the default key/agent or identity file, storing no password at all. Password entries still work and are captured once, stored as reversible Base64 in `servers.py`, protected by `0700`/`0600` modes, decoded only for `SSHPASS=... sshpass -e`, and never placed in argv. `tests/tests_security_hardening_defaults.sh` covers both paths. The PandoraNext example credentials were removed on 2026-07-19 as unused. | Base64 is not encryption; anyone who can read the cluster file can recover a stored password. Key-auth entries avoid this entirely. | Mitigated on 2026-07-19: SSH-key auth is available and stores no secret. Password storage remains for compatibility; existing Base64 entries are unchanged and administrators can switch a server to key auth by clearing its password (add) or editing `servers.py` (option 3). Host verification still follows the centralized `SSH_STRICT_HOST_KEY_CHECKING` policy (accept-new by default; set to `yes` with pre-provisioned known hosts to harden). |
...
| CMD-012 | High: cron is rewritten in many feature paths | Shared `cron_install_tagged`/`cron_remove_tagged` helpers now tag every entry they manage with `# ming-sh:<name>` and remove by that exact tag. The firewall persistence, rsync, and traffic-limit reset jobs were already tagged/exact; the logrotate, certificate-renewal, and TG-monitor writers were migrated on 2026-07-19, removing their broad `grep -v 'logrotate'` and `grep -v '~/TG-check-notify.sh'` filters. `tests/tests_cron_tagging.sh` enforces the helpers and forbids the removed filters. On 2026-07-19 the FRP (`grep -v 'frps'`/`'frpc'`) and OpenClaw gateway (`grep -v "s gateway"`) filters were found to have **no matching add site anywhere** — FRP is managed through Docker/tmux and the gateway through `openclaw gateway`, never cron — so they were vestigial cleanup that could only delete unrelated user cron; all six were removed. The fail2ban-uninstall `CF-Under-Attack.sh` filter is filename-specific and low risk; the custom-command feature filters a user-supplied keyword by design. | Incorrect filters can delete unrelated jobs; arbitrary scheduled commands persist with the user's privileges. Existing installations may still carry untagged entries, which are manual cleanup per project policy. | Resolved for project-managed writers: co-located writers use tagged helpers and the vestigial broad-word filters are gone (`tests/tests_cron_tagging.sh` guards both). Only the by-design filters remain (filename-specific CF cleanup; user-supplied custom-command keyword). |
| CMD-013 | High: dependencies are mutable and usually unpinned | Installers use `latest`, branch heads, short domains, mutable images, and remote application definitions. The remote-script validator displays the fetched SHA-256 but does not compare it with a repository-pinned value. A 2026-07-19 review of the whole `run_reviewed_remote_script` surface (50 distinct URL expressions across 94 call sites; re-counted 2026-08-18 — an earlier "19 URLs" figure counted only the two classified subsets) found static script pinning is not applicable: 14 target third-party moving refs (`latest` release assets or `main`/`master` branch heads) that change by design, 5 interpolate the proxy-dependent `PROJECT_DOWNLOAD_BASE`, and the remaining 31 are likewise mutable third-party refs. The OpenSSH helper uses signed distribution packages instead of source tarballs. A follow-up review on 2026-07-19 of the image and release surface reached the same conclusion: 8 of 9 release-artifact downloads use `releases/latest/download` (only one, antibody `2.0.6`, is version-pinned), and every container image runs on a rolling tag (`:latest`/`:stable`/`:main`/`:alpine`), including the three centralized `UPSTREAM_*_IMAGE` values. None carries a stable digest to pin. | A future upstream change can alter behavior without a repository change and can execute as soon as its feature is selected. | Reclassified and closed for pinning on 2026-07-19: neither scripts, images, nor releases have a stable digest to pin — the surface is mutable by design (users install the *latest* app), and freezing digests would break that intent and create a stale-image/patch liability. The applicable controls are transport (HTTPS, done) and the one already version-pinned artifact. Residual risk is inherent to user-selected latest-artifact installs and accepted; the `kjlion` upstream images remain a documented supply-chain dependency (switching to official images is a separate functional decision, not an integrity pin). |
| CMD-019 | High: interactive values reached sed replacements unescaped | `palworld.sh` spliced the interactive server password and experience-rate values into `/`-delimited `sed -i "s/.../...$value/"` replacements; a `/` in the value terminated the substitution and, on GNU sed, an injected `e` command could execute shell commands with the script's privileges. Since 2026-08-18 the password is written through a `\|&`-escaped, `|`-delimited replacement (`sed_escape_replacement`), the exp-rate input is case-normalized to a numeric default, and the sibling parameters remain whitelist-mapped. `tests/tests_command_construction_safety.sh` rejects the former forms. | Previously a crafted in-game password or rate value could break out of the sed replacement and run commands as the invoking user (typically root). | Resolved on 2026-08-18. Arbitrary password characters remain usable; non-numeric rate input falls back to the documented default. |
| CMD-020 | High: project-owned cron scripts were fetched without the validator | `auto_cert_renewal.sh`, `CF-Under-Attack.sh`, `Limiting_Shut_down1.sh`, and the TG notify pair were raw-`curl`ed into `~`, chmodded, and persisted via cron/tmux/profile — executed remote scripts outside `run_reviewed_remote_script`. Since 2026-08-18 all five install through `download_reviewed_remote_script --install` (HTTPS-only, private cache, `bash -n`, digest display, then move into place). The Flarum Composer installer, formerly fetched via unverified `php -r copy()`, is now verified against the published `installer.sig` SHA-384 digest inside the container before execution. `tests/tests_command_construction_safety.sh` guards every site. | A compromised or hijacked project download path could previously ship a script straight into a root cron job with no syntax check or digest record. | Resolved on 2026-08-18. Installed filenames, cron entries, and the `.profile` line are unchanged; only the fetch path gained validation. |
...
| CMD-015 | Medium: test harnesses have unequal isolation | Safe shell tests use repository-local temporary trees and command stubs; `run_openclaw_manager_matrix.sh` still starts Docker containers and may pull images, so it stays out of the default push/PR validation. It now runs only through the `docker-matrix.yml` workflow, which is `workflow_dispatch`-only (manual trigger) and executes in a disposable GitHub-hosted runner. | Running the Docker matrix can modify Docker state or require network access. | Resolved on 2026-07-19: the matrix is isolated to a manual, disposable CI environment and never runs on the developer host or in default validation. |
...
bash tests/tests_readme_crosslinks.sh
...
Integrity pinning (originally item 1) was assessed across the whole surface on
2026-07-19 and closed: scripts, images, and releases are all deliberately
mutable (`latest`/branch heads/rolling tags), so there is no stable digest to
pin without breaking the "install the latest app" intent and creating a
stale-image liability. HTTPS transport, syntax validation, and digest display
are the applicable controls (see CMD-002/CMD-013). The 2026-08-18 acceptance
review closed two further items: unescaped interactive values in `palworld.sh`
sed replacements (CMD-019) and unvalidated fetch-and-persist of project-owned
cron scripts plus the Composer installer (CMD-020). The remaining actionable
items:

1. Continue replacing feature-specific recursive deletion with canonical-path
   guards and practical rollback/backup behavior without duplicating existing
   feature confirmations.
2. Replace password-based cluster and backup automation with SSH keys or an
   approved secret store, and prefer `StrictHostKeyChecking=yes` with
   pre-provisioned known hosts. (Cluster SSH-key auth landed under CMD-010 on
   2026-07-19; `beifen.sh` and backup automation remain.)
3. Replace third-party fixed initial credentials with generated first-run
   secrets where the upstream application permits it.
4. Reduce supply-chain exposure to the `kjlion` upstream images by evaluating a
   switch to official base images where the project does not depend on their
   customizations (a functional decision, not an integrity pin).
