...
| CMD-010 | High: credentials are stored or displayed in plaintext | Cluster passwords are captured once, stored as reversible Base64 in `servers.py` for automation, protected by directory/file modes `0700`/`0600`, decoded only for `SSHPASS=... sshpass -e`, and not placed in argv. PandoraNext again ships network/signup/server-token functionality plus upstream example setup/token credentials. | Base64 is not encryption; anyone who can read the cluster file can recover passwords. PandoraNext example credentials are public and unsafe until replaced. | Open by compatibility design on 2026-07-14. File permissions and process-list handling are mitigations, not secret storage; administrators should replace examples and migrate clusters to SSH keys. |
