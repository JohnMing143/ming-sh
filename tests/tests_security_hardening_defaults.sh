...
for script in "${implementations[@]}"; do
	...
	grep -Fq 'server_credential="base64:$server_password_encoded"' "$script" || fail "cluster password is not persisted for automation: $script"
	grep -Fq '\"password\": \"$server_credential\"' "$script" || fail "cluster credential is not written to servers.py: $script"
	grep -Fq 'server_credential="key"' "$script" || fail "cluster SSH-key auth option is missing: $script"
	grep -Fq 'ssh -t -o StrictHostKeyChecking="$SSH_STRICT_HOST_KEY_CHECKING" "$username@$hostname"' "$script" || fail "cluster key-auth path does not use ssh without a stored password: $script"
