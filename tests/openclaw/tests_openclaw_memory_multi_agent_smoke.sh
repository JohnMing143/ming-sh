...
WORKDIR=$(mktemp -d "$REPO_ROOT/.openclaw-memory-multi-agent-test.XXXXXX")
...
[
  ...
  },
  {
    ...
    "status": {
      ...
      "dirty": false,
      "vector": {"enabled": true, "available": true},
      "workspaceDir": "${HOME}/.openclaw/workspace-work",
      "dbPath": "${HOME}/.openclaw/workspace-work/memory/index.sqlite"
    },
    "scan": {"issues": []}
  }
]
