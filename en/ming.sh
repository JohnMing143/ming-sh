...
# Telemetry is permanently disabled. Calls are retained as local no-ops so
# feature control flow and translated call sites keep their existing behavior.
send_stats() {
	return 0
}
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}hint:${gl_bai}This function requires root user to run!" && break_end && return_to_main_menu
}
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# Mode:" "$ACTIVE_OPTIMIZE_CONFIG_PATH" 2>/dev/null | sed 's/# mode: //' | awk -F'|' '{print $1}' | xargs)
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 OPENCLAW Management Tool by ${PROJECT_NAME} 🦞"
		echo -e "💡 Terminal execution \033[1;33m${PROJECT_COMMAND} claw\033[0m to quickly enter the menu"
...
def send_stat(_action):
    """Compatibility no-op: telemetry is permanently disabled."""
    return
...
EOF
	...
	openclaw_memory_view_file() {
		...
		read -e -p "Please enter the starting line (Press Enter to default to the end of ${default_lines} lines):" start_line
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "set up${docker_name}Login key (sk-a combination of letters and numbers starting with) such as: sk-example159163:" app_passwd
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}Advanced usage of ${PROJECT_COMMAND} command${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}Uninstall tech lion script"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}Return to main menu"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "Please enter your choice:" sub_choice
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "Please enter a new parsing record format: 110.25.5.33 example.com:" addhost
		  ...
		  100)
			  clear
			  echo "Privacy & Security / 隐私与安全"
			  echo "Telemetry and usage statistics are permanently disabled in ${PROJECT_NAME}."
			  echo "send_stats is a local no-op and no telemetry endpoint is configured."
			  break_end
			  ;;
		  ...
		  102)
			  ...
			  echo "------------------------------------------------"
			  echo "${PROJECT_NAME} will be completely uninstalled without affecting your other functions."
			  read -e -p "Are you sure you want to continue? (Y/N):" choice
...
command_help() {
send_stats "${PROJECT_COMMAND} command reference use case"
echo "-------------------"
echo "Video introduction: https://www.bilibili.com/video/BV1ib421E7it?t=0.1"
echo "The following is a reference use case for the ${PROJECT_COMMAND} command:"
echo "Start script ${PROJECT_COMMAND}"
echo "Install packages ${PROJECT_COMMAND} install nano wget | ${PROJECT_COMMAND} add nano wget | ${PROJECT_COMMAND} install nano wget"
echo "Uninstall a package ${PROJECT_COMMAND} remove nano wget | ${PROJECT_COMMAND} del nano wget | ${PROJECT_COMMAND} uninstall nano wget | ${PROJECT_COMMAND} uninstall nano wget"
echo "Update system ${PROJECT_COMMAND} update | ${PROJECT_COMMAND} update"
echo "Clean system junk ${PROJECT_COMMAND} clean | ${PROJECT_COMMAND} clean"
echo "Reinstall the system panel ${PROJECT_COMMAND} dd | ${PROJECT_COMMAND} reinstall"
echo "bbr3 control panel ${PROJECT_COMMAND} bbr3 | ${PROJECT_COMMAND} bbrv3"
echo "Kernel Tuning Panel ${PROJECT_COMMAND} nhyh | ${PROJECT_COMMAND} Kernel Optimization"
echo "Set virtual memory ${PROJECT_COMMAND} swap 2048"
echo "Set virtual time zone ${PROJECT_COMMAND} time Asia/Shanghai | ${PROJECT_COMMAND} time zone Asia/Shanghai"
echo "System Recycle Bin ${PROJECT_COMMAND} trash | ${PROJECT_COMMAND} hsz | ${PROJECT_COMMAND} Recycle Bin"
echo "System backup function ${PROJECT_COMMAND} backup | ${PROJECT_COMMAND} bf | ${PROJECT_COMMAND} backup"
echo "ssh remote connection tool ${PROJECT_COMMAND} ssh | ${PROJECT_COMMAND} remote connection"
echo "rsync remote synchronization tool ${PROJECT_COMMAND} rsync | ${PROJECT_COMMAND} remote synchronization"
echo "Hard disk management tool ${PROJECT_COMMAND} disk | ${PROJECT_COMMAND} hard disk management"
echo "Intranet penetration (server) ${PROJECT_COMMAND} frps"
echo "Intranet penetration (client) ${PROJECT_COMMAND} frpc"
echo "Software startup ${PROJECT_COMMAND} start sshd | ${PROJECT_COMMAND} start sshd"
echo "Software stop ${PROJECT_COMMAND} stop sshd | ${PROJECT_COMMAND} stop sshd"
echo "Software restart ${PROJECT_COMMAND} restart sshd | ${PROJECT_COMMAND} restart sshd"
echo "Check software status ${PROJECT_COMMAND} status sshd | ${PROJECT_COMMAND} status sshd"
echo "${PROJECT_COMMAND} enable docker | ${PROJECT_COMMAND} autostart docker | ${PROJECT_COMMAND} enable docker when booting the software"
echo "Domain name certificate application ${PROJECT_COMMAND} ssl"
echo "Domain name certificate expiry query ${PROJECT_COMMAND} ssl ps"
echo "docker management plane ${PROJECT_COMMAND} docker"
echo "docker environment installation ${PROJECT_COMMAND} docker install |${PROJECT_COMMAND} docker installation"
echo "docker container management ${PROJECT_COMMAND} docker ps |${PROJECT_COMMAND} docker container"
echo "docker image management ${PROJECT_COMMAND} docker img |${PROJECT_COMMAND} docker image"
echo "LDNMP site management ${PROJECT_COMMAND} web"
echo "LDNMP cache cleaning ${PROJECT_COMMAND} web cache"
echo "Install WordPress ${PROJECT_COMMAND} wp | ${PROJECT_COMMAND} wordpress | ${PROJECT_COMMAND} wp xxx.com"
echo "Install reverse proxy ${PROJECT_COMMAND} fd |${PROJECT_COMMAND} rp |${PROJECT_COMMAND} reverse proxy |${PROJECT_COMMAND} fd xxx.com"
echo "Install load balancing ${PROJECT_COMMAND} loadbalance |${PROJECT_COMMAND} load balancing"
echo "Install L4 load balancing ${PROJECT_COMMAND} stream |${PROJECT_COMMAND} L4 load balancing"
echo "firewall panel ${PROJECT_COMMAND} fhq |${PROJECT_COMMAND} firewall"
echo "open port ${PROJECT_COMMAND} dkdk 8080 |${PROJECT_COMMAND} open port 8080"
echo "Close port ${PROJECT_COMMAND} gbdk 7800 |${PROJECT_COMMAND} Close port 7800"
echo "Release IP ${PROJECT_COMMAND} fxip 127.0.0.0/8 |${PROJECT_COMMAND} Release IP 127.0.0.0/8"
echo "Block IP ${PROJECT_COMMAND} zzip 177.5.25.36 |${PROJECT_COMMAND} Block IP 177.5.25.36"
echo "command favorites ${PROJECT_COMMAND} fav | ${PROJECT_COMMAND} command favorites"
echo "Application market management ${PROJECT_COMMAND} app"
echo "Quick management of application numbers ${PROJECT_COMMAND} app 26 | ${PROJECT_COMMAND} app 1panel | ${PROJECT_COMMAND} app npm"
echo "fail2ban management ${PROJECT_COMMAND} fail2ban | ${PROJECT_COMMAND} f2b"
echo "Display system information ${PROJECT_COMMAND} info"
echo "ROOT key management ${PROJECT_COMMAND} sshkey"
echo "SSH public key import (URL) ${PROJECT_COMMAND} sshkey <url>"
echo "SSH public key import (GitHub) ${PROJECT_COMMAND} sshkey github <user>"
		...
		sshkey)
			...
			case "$1" in
				...
				* )
					...
					echo "usage:"
					echo "${PROJECT_COMMAND} sshkey enters the interactive menu"
					echo "${PROJECT_COMMAND} sshkey \"<pubkey>\" Directly import the SSH public key"
					echo "${PROJECT_COMMAND} sshkey <url> Import SSH public key from URL"
					echo "${PROJECT_COMMAND} sshkey github <user> Import SSH public key from GitHub"
					;;
			esac
