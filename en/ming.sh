	  ...
	  case $sub_choice in
		  1)
			  while true; do
				  ...
				  install_project_entrypoint || {
					  echo -e "${gl_hong}Command installation failed; no shortcut was created.${gl_bai}"
					  break_end
					  continue
				  }
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path_interactive "$dirname" && echo "Directory deleted" || echo "Delete failed"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
...
run_commands_on_servers() {
	...
	for ((i=0; i<${#SERVER_ARRAY[@]}; i+=5)); do
		...
		if [ -n "$stored_password" ]; then echo -e "${gl_hong}A legacy plaintext password was ignored; clear it from servers.py.${gl_bai}"; fi
		kj_ssh_read_password "Password for $username@$hostname: "
