...
download_reviewed_remote_script() {
	...
	case "$script_url" in
		https://*) ;;
		*) echo "HTTPS가 아닌 스크립트 다운로드 거부: $script_url"; return 1 ;;
	esac
	cache_dir="${PROJECT_CACHE_DIR:-${XDG_CACHE_HOME:-${HOME}/.cache}/${PROJECT_ID:-ming-sh}}/remote-scripts"
	[ ! -L "$cache_dir" ] || { echo "심볼릭 링크 캐시 디렉터리 사용을 거부합니다.$cache_dir"; return 1; }
	mkdir -p -- "$cache_dir" || return 1
	[ -O "$cache_dir" ] || { echo "원격 스크립트 캐시 디렉터리가 현재 사용자 소유가 아님: $cache_dir"; return 1; }
	...
	[ -s "$script_path" ] || { echo "다운로드 결과가 비어 있습니다."; rm -f -- "$script_path"; return 1; }
	bash -n "$script_path" || { echo "원격 스크립트가 Bash 구문 검사에 실패했습니다:$script_path"; rm -f -- "$script_path"; return 1; }
	...
	printf '원격 스크립트가 HTTPS 다운로드 및 Bash 구문 검사를 통과했으며 자동으로 실행됩니다.\n출처: %s\nSHA-256: %s\n' "$script_url" "$digest"
	bash "$script_path" "$@"
	exit_status=$?
	rm -f -- "$script_path"
	return "$exit_status"
...
install_project_entrypoint() {
	...
	if [ -f "$PROJECT_HOME_PATH" ] && [ "$source_real" != "$home_real" ] && ! project_entrypoint_is_managed_file "$PROJECT_HOME_PATH"; then
		echo "프로젝트가 아닌 기본 스크립트 덮어쓰기 거부: $PROJECT_HOME_PATH"
		return 1
	fi
...
UserLicenseAgreement() {
	...
	if [ "$user_input" = "y" ] || [ "$user_input" = "Y" ]; then
		[ ! -L "$PROJECT_STATE_DIR" ] || { echo "Refusing a symlinked state directory."; exit 1; }
		mkdir -p -- "$PROJECT_STATE_DIR" || exit 1
		[ -O "$PROJECT_STATE_DIR" ] || { echo "The state directory is not owned by the current user."; exit 1; }
...
# 원래의 기본 명령 설치 동작을 유지합니다. 보안 검사에 실패하더라도 주요 기능을 차단하지 마십시오.
		...
		1)
			...
			if [ "${#docker_command[@]}" -lt 2 ] ||
			   ...
			   { [ "${docker_command[1]}" != "run" ] && [ "${docker_command[1]}" != "create" ]; }; then
				echo -e "${gl_hong}docker run 또는 docker create 명령만 실행할 수 있습니다.${gl_bai}"
			else
				"${docker_command[@]}"
			fi
			;;
...
# 광범위한 grep 필터를 사용하여 관련 없는 작업을 실수로 삭제하는 것을 방지하기 위해 프로젝트 태그로 cron 작업을 관리합니다.
# 표시는 "# ming-sh:<이름>" 형식으로 되어있습니다. 설치시 표시를 누르면 중복 제거 후 추가되고, 제거시에는 표시를 누르면 정확하게 삭제됩니다.
cron_install_tagged() {
...
iptables_open() {
	install iptables
	backup_iptables_rules || echo -e "${gl_huang}경고: 방화벽 규칙의 자동 백업이 실패했습니다. 계속해서 모든 포트를 여십시오.${gl_bai}"
...
iptables_close_all() {
	local current_port
	install iptables
	backup_iptables_rules || echo -e "${gl_huang}경고: 방화벽 규칙의 자동 백업이 실패했습니다. 계속해서 모든 포트를 닫습니다.${gl_bai}"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}힌트:${gl_bai}이 기능을 실행하려면 루트 사용자가 필요합니다!" && break_end && return_to_main_menu
}
...
clamav_scan() {
	...
	echo -e "${gl_lv}$* 스캔 완료, 바이러스 보고서는 ${gl_huang}/home/docker/clamav/log/scan.log${gl_bai}에 있습니다"
...
add_connection() {
	...
	kj_ssh_read_host_user_port "IP 주소를 입력하세요:" "사용자 이름을 입력하십시오(기본값: 루트):" "请输入端口号 (默认: 22): " "root" "22"
...
format_partition() {
	...
	# 포맷 확인
	read -e -r -p "$DEVICE 파티션을 $FS_TYPE 형식으로 포맷하시겠습니까? (y/n): " CONFIRM
	if [[ ! "$CONFIRM" =~ ^[Yy]$ ]]; then
		echo "작업이 취소되었습니다."
		return
	fi
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 ${PROJECT_NAME}의 OPENCLAW 관리 도구 🦞"
...
print(f'✅ {target}: {len(add_ids)} 추가, {len(removed_ids)} 삭제, 현재 {len(new_models)}')
...
PY
	...
	openclaw_memory_recommend() {
		...
		case "$model_status" in
			ok) OPENCLAW_MEMORY_RECOMMEND_REASON+=("로컬 모델 파일이 존재함") ;;
...
EOF
	...
	openclaw_memory_view_file() {
		...
		read -e -p "시작 줄을 입력하세요(Enter를 누르면 마지막 ${default_lines}개 줄이 기본값): " start_line
		read -e -p "표시할 줄 수를 입력하세요(Enter를 누르면 기본값 ${default_lines}): " count
...
' "$allowlist_json"
			...
			case "$al_choice" in
				1)
					read -e -p "해제할 명령 경로를 입력하세요(/usr/bin/git와 같은 glob 지원)." pattern
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_minglan}115. ${color115}헤르메스 로봇 관리 도구${gl_huang}★${gl_bai}"
	  echo -e "${gl_minglan}-------------------------"
	...
	case $sub_choice in
	  ...
	  41|haozi|acepanel)
		...
		local app_id="41"
		local lujing="[ -d "/www/server/panel" ]"
		local panelname="AcePanel (구 Haozi 패널)"
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "설정${docker_name}로그인 키(sk-다음과 같이 시작하는 문자와 숫자의 조합): sk-example159163:" app_passwd
...
linux_work() {
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  23)
			  read -e -p "백그라운드에서 실행할 명령을 입력하십시오(예: uptime):" tmuxd
			  tmux_run_d
			  ;;
		  ...
		  1)
			  while true; do
				  ...
				  if [[ ! "$kuaijiejian" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || [[ "$kuaijiejian" == *..* ]]; then
					  echo -e "${gl_hong}바로가기 이름에는 영문자, 숫자, 점, 밑줄 및 하이픈만 사용할 수 있으며 연속된 점은 사용할 수 없습니다.${gl_bai}"
					  break_end
					  continue
				  fi
				  ...
				  for shortcut_path in "/usr/local/bin/$kuaijiejian" "/usr/bin/$kuaijiejian"; do
					  if [ -e "$shortcut_path" ] || [ -L "$shortcut_path" ]; then
						  ...
						  if [ "$shortcut_target" != "$project_install_target" ]; then
							  echo -e "${gl_hong}기존 명령을 덮어쓰지 않습니다: $shortcut_path${gl_bai}"
				  ...
				  install_project_entrypoint || {
					  echo -e "${gl_hong}명령 설치에 실패했습니다.${gl_bai}"
					  break_end
					  continue
				  }
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "새로운 구문 분석 기록 형식을 입력하세요: 110.25.5.33 example.com:" addhost
		  ...
		  102)
			  clear
			  echo "${PROJECT_NAME} 제거"
			  echo "------------------------------------------------"
			  echo "${PROJECT_NAME}은 다른 기능에 영향을 주지 않고 완전히 제거됩니다."
			  read -e -p "계속하시겠습니까? (예/아니요):" choice
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path "$dirname" && echo "디렉터리가 삭제되었습니다." || echo "삭제 실패"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
...
while true; do
	  ...
	  case $sub_choice in
		  1)
			  ...
			  echo -e "${gl_hui}비밀번호를 비워두면 SSH 키 인증(기본 키 또는 ssh-agent)이 사용됩니다.${gl_bai}"
...
echo "다음은 ${PROJECT_COMMAND} 명령의 참조 사용 사례입니다."
echo "시작 스크립트 ${PROJECT_COMMAND}"
echo "패키지 설치 ${PROJECT_COMMAND} install nano wget | ${PROJECT_COMMAND} 나노 wget 추가 | ${PROJECT_COMMAND}는 나노 wget을 설치합니다."
echo "패키지 제거 ${PROJECT_COMMAND} 제거 nano wget | ${PROJECT_COMMAND} 델 나노 wget | ${PROJECT_COMMAND} 나노 wget 제거 | ${PROJECT_COMMAND} 나노 wget 제거"
echo "시스템 ${PROJECT_COMMAND} 업데이트 업데이트 | ${PROJECT_COMMAND} 업데이트"
echo "클린 시스템 정크 ${PROJECT_COMMAND} 클린 | 케이 깨끗하다"
echo "시스템 패널을 다시 설치하십시오. ${PROJECT_COMMAND} dd | ${PROJECT_COMMAND} 다시 설치하다"
echo "bbr3 제어판 ${PROJECT_COMMAND} bbr3 | kbbrv3"
echo "커널 튜닝 패널 ${PROJECT_COMMAND} nhyh | ${PROJECT_COMMAND} 커널 최적화"
echo "가상 메모리 ${PROJECT_COMMAND} 스왑 2048 설정"
echo "가상 시간대 설정 ${PROJECT_COMMAND} 시간 아시아/상하이 | ${PROJECT_COMMAND} 시간대 아시아/상하이"
echo "시스템 휴지통 ${PROJECT_COMMAND} 쓰레기 | khsz | ${PROJECT_COMMAND} 휴지통"
echo "시스템 백업 기능 ${PROJECT_COMMAND} 백업 | ${PROJECT_COMMAND} bf | ${PROJECT_COMMAND} 백업"
echo "SSH 원격 연결 도구 ${PROJECT_COMMAND} SSH | ${PROJECT_COMMAND} 원격 연결"
echo "rsync 원격 동기화 도구 ${PROJECT_COMMAND} rsync | ${PROJECT_COMMAND} 원격 동기화"
echo "하드 디스크 관리 도구 ${PROJECT_COMMAND} 디스크 | ${PROJECT_COMMAND} 하드 디스크 관리"
echo "인트라넷 침투(서버) ${PROJECT_COMMAND} frps"
echo "인트라넷 침투(클라이언트) ${PROJECT_COMMAND} frpc"
echo "소프트웨어 시작 ${PROJECT_COMMAND} start sshd | ${PROJECT_COMMAND} sshd를 시작하세요"
echo "소프트웨어 중지 ${PROJECT_COMMAND} 중지 sshd | ${PROJECT_COMMAND} sshd 중지"
echo "소프트웨어 재시작 ${PROJECT_COMMAND} 재시작 sshd | ${PROJECT_COMMAND} sshd를 다시 시작하세요"
echo "소프트웨어 상태 확인 ${PROJECT_COMMAND} status sshd | ${PROJECT_COMMAND} 상태 sshd"
echo "${PROJECT_COMMAND} 도커 활성화 | ${PROJECT_COMMAND} 자동 시작 도커 | ${PROJECT_COMMAND} 소프트웨어를 부팅할 때 도커를 활성화합니다."
echo "도메인 이름 인증서 신청 ${PROJECT_COMMAND} SSL"
echo "도메인 이름 인증서 만료 쿼리 ${PROJECT_COMMAND} SSL ps"
echo "도커 관리 플레인 ${PROJECT_COMMAND} 도커"
echo "docker 환경 설치 ${PROJECT_COMMAND} docker 설치 |${PROJECT_COMMAND} docker 설치"
echo "도커 컨테이너 관리 ${PROJECT_COMMAND} 도커 ps |${PROJECT_COMMAND} 도커 컨테이너"
echo "도커 이미지 관리 ${PROJECT_COMMAND} docker img |${PROJECT_COMMAND} 도커 이미지"
echo "LDNMP 사이트 관리 ${PROJECT_COMMAND} web"
echo "LDNMP 캐시 정리 ${PROJECT_COMMAND} 웹 캐시"
echo "WordPress ${PROJECT_COMMAND} wp 설치 | ${PROJECT_COMMAND} 워드프레스 | ${PROJECT_COMMAND} wp xxx.com"
echo "역방향 프록시 설치 ${PROJECT_COMMAND} fd |${PROJECT_COMMAND} rp |${PROJECT_COMMAND} 역방향 프록시 |${PROJECT_COMMAND} fd xxx.com"
echo "로드 밸런싱 설치 ${PROJECT_COMMAND} loadbalance |${PROJECT_COMMAND} 로드 밸런싱"
echo "L4 로드 밸런싱 설치 ${PROJECT_COMMAND} 스트림 |${PROJECT_COMMAND} L4 로드 밸런싱"
echo "방화벽 패널 ${PROJECT_COMMAND} fhq |${PROJECT_COMMAND} 방화벽"
echo "포트 ${PROJECT_COMMAND} 열기 DKdk 8080 |${PROJECT_COMMAND} 포트 8080 열기"
echo "${PROJECT_COMMAND} 포트 닫기 gbdk 7800 |${PROJECT_COMMAND} 포트 7800 닫기"
echo "릴리스 IP ${PROJECT_COMMAND} fxip 127.0.0.0/8 |${PROJECT_COMMAND} 릴리스 IP 127.0.0.0/8"
echo "IP 차단 ${PROJECT_COMMAND} zzip 177.5.25.36 |${PROJECT_COMMAND} IP 177.5.25.36 차단"
echo "명령 즐겨찾기 ${PROJECT_COMMAND} 즐겨찾기 | ${PROJECT_COMMAND} 명령 즐겨찾기"
echo "애플리케이션 시장 관리 ${PROJECT_COMMAND} app"
echo "신청번호의 빠른 관리 ${PROJECT_COMMAND} app 26 | ${PROJECT_COMMAND} 앱 1패널 | ${PROJECT_COMMAND} 앱 npm"
echo "Fail2ban 관리 ${PROJECT_COMMAND} Fail2ban | 케이 F2B"
echo "시스템 정보 표시 ${PROJECT_COMMAND} 정보"
echo "ROOT 키 관리 ${PROJECT_COMMAND} sshkey"
echo "SSH 공개 키 가져오기(URL) ${PROJECT_COMMAND} sshkey <url>"
echo "SSH 공개 키 가져오기(GitHub) ${PROJECT_COMMAND} sshkey github <user>"
		...
		sshkey)
			...
			case "$1" in
				...
				* )
					...
					echo "용법:"
					echo "${PROJECT_COMMAND} sshkey가 대화형 메뉴로 들어갑니다."
					echo "${PROJECT_COMMAND} sshkey \"<pubkey>\" SSH 공개 키 직접 가져오기"
					echo "${PROJECT_COMMAND} sshkey <url> URL에서 SSH 공개 키 가져오기"
					echo "${PROJECT_COMMAND} sshkey github <user> GitHub에서 SSH 공개 키 가져오기"
					;;
			esac
