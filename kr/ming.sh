	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				safe_remove_path_interactive "$dirname" && echo "디렉터리가 삭제되었습니다." || echo "삭제 실패"
				;;
			6)  # 返回上一级选单目录
				cd ..
				;;
...
run_commands_on_servers() {
	...
	for ((i=0; i<${#SERVER_ARRAY[@]}; i+=5)); do
		...
		if [ -n "$stored_password" ]; then echo -e "${gl_hong}이전 평문 비밀번호는 무시되었습니다. servers.py에서 삭제하세요.${gl_bai}"; fi
		kj_ssh_read_password "$username@$hostname 비밀번호: "
